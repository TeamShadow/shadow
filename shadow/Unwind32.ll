...
aligned:
	...
	%p.up = add i32 %p.int, sub (i32 ptrtoint (i1** getelementptr (i1*, i1** null, i32 1) to i32), i32 1)
	%p.round = and i32 %p.up, sub (i32 0, i32 ptrtoint (i1** getelementptr (i1*, i1** null, i32 1) to i32))
...
ttype.read:
	...
	%ttype.offset = load %_Unwind_Word, %_Unwind_Word* %temp.word, align 4
	%ttype.value = getelementptr i8, i8* %p.4, i32 %ttype.offset
	br label %ttype.done
ttype.done:
	...
	%action_table.offset = load %_Unwind_Word, %_Unwind_Word* %temp.word, align 4
	%action_table.base = getelementptr i8, i8* %p.7, i32 %action_table.offset
...
find.action.loop:
	...
	%action_record.offset = load %_Unwind_Word, %_Unwind_Word* %temp.word, align 4
	%code_section.start.int_offset = ptrtoint %_Unwind_Ptr %code_section.start.offset to i32
	%code_section.start = getelementptr i8, %_Unwind_Ptr %start, i32 %code_section.start.int_offset
...
find.action.check:
	...
	%code_section.end = getelementptr i8, %_Unwind_Ptr %code_section.start, i32 %code_section.end.int_offset
...
landing_pad.valid:
	...
	%landing_pad = getelementptr i8, %_Unwind_Ptr %landing_pad.start, i32 %landing_pad.int_offset
...
action_record.valid:
	%action_record = getelementptr i8, i8* %action_table, i32 %action_record.offset
...
thrown.class.get:
	...
	;%thrown.class.value = getelementptr inbounds %shadow.standard..Object_class, %shadow.standard..Object_class* %thrown.class.load, i32 0, i32 0
	br label %find.catch
find.catch:
	...
	%filter = load %_Unwind_Sword, %_Unwind_Sword* %temp.sword, align 4
...
find.catch.next:
	...
	%action_record.next = load %_Unwind_Sword, %_Unwind_Sword* %temp.sword, align 4
...
action_record.next.valid:
	%p.17 = getelementptr i8, i8* %p.14, %_Unwind_Sword %action_record.next
	br label %find.catch
found.catch:
...
entry:
	%1 = tail call noalias i8* @malloc(i32 add (i32 ptrtoint (%struct._Unwind_Exception* getelementptr (%struct._Unwind_Exception, %struct._Unwind_Exception* null, i32 1) to i32), i32 ptrtoint (i1** getelementptr (i1*, i1** null, i32 1) to i32))) nounwind
	...
	%4 = getelementptr %struct._Unwind_Exception, %struct._Unwind_Exception* %2, i32 0, i32 0
	store %_Unwind_Exception_Class %3, %_Unwind_Exception_Class* %4
	%5 = getelementptr %struct._Unwind_Exception, %struct._Unwind_Exception* %2, i32 0, i32 1
	store %_Unwind_Exception_Cleanup_Fn @shadow.exception.cleanup, %_Unwind_Exception_Cleanup_Fn* %5
	%6 = getelementptr %struct._Unwind_Exception, %struct._Unwind_Exception* %2, i32 1
...
define noalias %shadow.standard..Object* @__allocate(%shadow.standard..Class* %class, %shadow.standard..Object_methods* %methods) {	
	...
	%memory = call noalias i8* @calloc(%uint 1, %uint %size) nounwind
...
define noalias %shadow.standard..Array* @__allocateArray(%shadow.standard..GenericClass* %class, %ulong %longElements, %boolean %nullable) {	
...
@_array0 = private unnamed_addr constant {%ulong, %shadow.standard..Class*, %shadow.standard..Array_methods*, %long, [20 x %ubyte]} {%ulong -1, %shadow.standard..Class* @ubyte_A_class, %shadow.standard..Array_methods* @shadow.standard..Array_methods, %long 20, [20 x %ubyte] c"Heap space exhausted"}
@_string0 = private unnamed_addr constant %shadow.standard..String { %ulong -1, %shadow.standard..Class* @shadow.standard..String_class, %shadow.standard..String_methods* @shadow.standard..String_methods, %shadow.standard..Array* bitcast ( {%ulong, %shadow.standard..Class*, %shadow.standard..Array_methods*, %long, [20 x %ubyte]}* @_array0 to %shadow.standard..Array*), %boolean true }
