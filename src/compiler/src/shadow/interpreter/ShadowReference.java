...
public class ShadowReference implements ModifiedType
{
	...
	public void setValue(ShadowValue value) throws ShadowException
	{
		if (value.getModifiers().isNullable() && !getModifiers().isNullable())
			throw new ShadowException("Cannot store a nullable value to a " +
					"non-nullable reference");
		if (value.getModifiers().isReadonly() && !getModifiers().isReadonly())
			throw new ShadowException("Cannot store a readonly value to a " +
					"non-readonly reference");
		if (value.getModifiers().isImmutable() != getModifiers().isImmutable())
			if (getModifiers().isImmutable())
				throw new ShadowException("Cannot store a non-immutable " +
						"value to an immutable reference");
			else if (!getModifiers().isReadonly())
				throw new ShadowException("Cannot store an immutable value " +
						"to a non-readonly non-immutable reference");
		if (!value.getType().isSubtype(getType()))
			throw new ShadowException("Cannot store a value of type " +
					value.getType() + " to a reference of type " + getType());
		this.value = value;
	}
}
