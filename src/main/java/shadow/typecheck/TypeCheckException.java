...
public class TypeCheckException extends ShadowException implements Comparable<TypeCheckException> {
  ...
  public enum Error implements ShadowExceptionErrorKind {
    ...
    INVALID_HIERARCHY(
        "Invalid hierarchy", "Type contains a circular extends or implements definition"),
    ...
    INVALID_METHOD(
        "Invalid method", "Method with specified signature is not defined in this context"),
    ...
    MISSING_CREATE(
        "Missing create invocation",
        "Explicit create invocation is missing, and parent class does not implement the default create"),
    ...
    MULTIPLY_DEFINED_SYMBOL(
        "Multiply defined symbol", "Symbol cannot be redefined in this context"),
    ...
    NOT_ALL_PATHS_RETURN(
        "Not all paths return", "Every path in a value-returning method must return"),
    READ_OF_THIS_IN_CREATE(
        "Read of this in create",
        "Field that might contain a reference to \"this\" cannot be read in a create or methods called by a create"),
    ...
    UNINITIALIZED_FIELD(
        "Uninitialized field", "Non-nullable field may not be initialized by a create"),
    ...
    INVALID_METHOD_IDENTIFIER(
        "Invalid identifier", "Only assembly import methods can start with an underscore."),
    INVALID_METHOD_IMPORT(
        "Invalid method import",
        "The referenced class does not contain a method which matches this method's signature."),
    REPEATED_ASSIGNMENT(
        "Repeated assignment",
        "Attribute fields may only be assigned once per attribute invocation"),
    REPEATED_ATTRIBUTE(
        "Repeated annotation",
        "Multiple instances of the same attribute type cannot be applied to the same method");
  ...
  /**
   * Creates a <code>TypeCheckException</code> with a specified kind of error and a message in a
   * particular context.
   *
   * @param kind kind of error
   * @param message explanatory error message
  ...
  public int compareTo(TypeCheckException other) {
    ...
    if (difference == 0) difference = getMessage().compareTo(other.getMessage());
    return difference;
  }
}
