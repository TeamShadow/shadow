...
/**
 * The <code>TypeUpdater</code> class updates types after they have been
 * collected by the <code>TypeCollector</code>. This class adds fields and
 * methods, instantiates type parameters, adds parent types and interfaces, and
 * checks method overrides. It fills in the general shape of all of the types
 * but does not check the actual statements of executable code.
 * 
...
public class TypeUpdater extends BaseChecker {
	...
	/**
	 * Updates all of the types in the given node table and returns and updated
	 * table. This central method of the class performs all of its functions.
	 * 
	 * @param typeTable
	 *            map from types to nodes
	 * @return
	...
	private void addCreatesAndProperties(Context declaration) {
		...
		if (type instanceof ClassType) {
			...
			if (classType.getMethodOverloads("create").isEmpty()) {
				...
				MethodSignature createSignature = new MethodSignature(classType, "create", createNode.getModifiers(),
						createNode.getDocumentation(), createNode);
			...
			if (classType.getMethodOverloads("destroy").isEmpty()) {
				...
				MethodSignature destroySignature = new MethodSignature(classType, "destroy", destroyNode.getModifiers(),
						destroyNode.getDocumentation(), destroyNode);
			...
			MethodSignature copySignature = new MethodSignature(classType, "copy", copyNode.getModifiers(),
					copyNode.getDocumentation(), copyNode);
				...
				if (fieldModifiers.isGet() || fieldModifiers.isSet()) {
					...
					for (MethodSignature signature : methods) {
						if (signature.getModifiers().isGet()) {
							...
							getterCollision++;
								...
								&& field.getValue().getType().equals(signature.getParameterTypes().get(0).getType()))
							setterCollision++;
					}

					if (fieldModifiers.isGet() && getterCount == 0) {
						if (getterCollision > 0)
							addError(node, Error.INVALID_MODIFIER, "Default get property " + field.getKey()
									+ " cannot replace a non-get method with an indistinguishable signature");
						else {
							ShadowParser.MethodDeclarationContext methodNode = new ShadowParser.MethodDeclarationContext(
									null, -1);
							...
							SimpleModifiedType modifiedType = new SimpleModifiedType(field.getValue().getType(),
									modifiers);
					...
					if (fieldModifiers.isSet() && setterCount == 0) {
						if (setterCollision > 0)
							addError(node, Error.INVALID_MODIFIER, "Default set property " + field.getKey()
	...
	private Map<Type, Context> instantiateTypeParameters(Map<Type, Context> nodeTable) {
		...
		try {
			...
			addError(nodeTable.get(type), Error.INVALID_TYPE_PARAMETERS,
					"Type " + type + " contains a circular type parameter definition", type);
		}
	...
	private List<Context> sortOnIsLists(Map<Type, Context> nodeTable) {
		...
		try {
			...
			addError(node, Error.INVALID_HIERARCHY, "Type " + node.getType() + " contains a circular is definition",
					node.getType());
		}
			...
			if (declarationNode.getType() instanceof ClassType) {
				...
				if (parent != null) {
					...
					if (classType.getModifiers().isImmutable()) {
						...
						for (MethodSignature signature : list)
							if (!signature.isCreate() && !signature.isDestroy()
									&& !signature.getModifiers().isReadonly())
								addError(signature.getNode(), Error.INVALID_METHOD,
										"Mutable parent method " + signature.getSymbol() + signature.getMethodType()
					...
					for (List<MethodSignature> signatures : classType.getMethodMap().values())
						for (MethodSignature signature : signatures) {
							if (parent.recursivelyContainsIndistinguishableMethod(signature) && !signature.isCreate()) {
								MethodSignature parentSignature = parent
										.recursivelyGetIndistinguishableMethod(signature);
								...
								if (parentModifiers.isLocked())
									addError(node, Error.INVALID_OVERRIDE, "Method " + signature
											+ " cannot override locked method " + parentSignature);
								else if (!parentSignature.getReturnTypes().canAccept(signature.getReturnTypes()))
									addError(node, Error.INVALID_OVERRIDE, "Overriding method " + signature
											+ " differs only by return type from " + parentSignature);
								else if (!modifiers.isReadonly() && parentModifiers.isReadonly())
									addError(node, Error.INVALID_OVERRIDE,
									...
									addError(node, Error.INVALID_OVERRIDE, "Overriding method " + signature
											+ " cannot reduce visibility of public method " + parentSignature);
								else if (parentModifiers.isProtected() && modifiers.isPrivate())
									addError(node, Error.INVALID_OVERRIDE, "Overriding method " + signature
											+ " cannot reduce visibility of protected method " + parentSignature);
							}
						}
				}
				...
				for (InterfaceType interfaceType : classType.getInterfaces()) {
					...
					if (classType.hasOuter() && interfaceType.recursivelyContainsMethod("create"))
						addError(declarationNode, Error.INVALID_INTERFACE,
								"Inner class " + classType + " cannot implement interface " + interfaceType
			...
			if (parent.recursivelyContainsInnerType(type)) {
				...
				while (outer != parent) {
					...
					if (outer.getModifiers().hasModifier(Modifiers.PRIVATE)
						...
						addError(method.getNode(), Error.ILLEGAL_ACCESS, "Method " + method.getSymbol()
								+ method.getMethodType() + " is more visible than return type " + type);
		...
		for (ModifiedType modifiedType : method.getParameterTypes()) {
			...
			if (parent.recursivelyContainsInnerType(type)) {
				...
				while (outer != parent) {
					...
					if (outer.getModifiers().hasModifier(Modifiers.PRIVATE)
							|| (isPublic && outer.getModifiers().hasModifier(Modifiers.PROTECTED))) {
						addError(method.getNode(), Error.ILLEGAL_ACCESS,
	...
	/**
	 * Tries to find a type without any other information. This method starts by
	 * looking in the current method, which will eventually look through outer
	 * types and then imported types. This method overrides the
	 * <code>BaseChecker</code> method and adds the ability to resolve type
	 * parameters.
	 * 
	 * @param name
	 *            name of the type
	...
	private boolean checkModifiers(Context node, String kind) {
		...
		if (currentType instanceof InterfaceType) {
			if (visibilityModifiers > 0) {
				addError(node, Error.INVALID_MODIFIER, "Interface " + kind
	...
	private void visitMethodPre(String name, Context node) {
		 ...
		 */
		MethodSignature signature = new MethodSignature(currentType, name, node.getModifiers(), node.getDocumentation(),
				node);
	...
	private void visitMethodPost(Context node) {
		...
		if (currentType instanceof ClassType) {
			if (isMeta && hasBlock) {
				addError(node, Error.INVALID_STRUCTURE,
			...
			if (!isMeta) {
				...
				if (hasBlock && (signature.isAbstract() || signature.isImport())) {
					addError(node, Error.INVALID_STRUCTURE, (signature.isAbstract() ? "Abstract" : "Import")
							+ " method " + signature + " must not define a body");
				}
	...
	public Void visitCreateDeclarator(ShadowParser.CreateDeclaratorContext ctx) {
		...
		if ((currentType instanceof SingletonType) && signature.getParameterTypes().size() > 0) {
			addError(ctx, Error.INVALID_SINGLETON_CREATE,
	...
	public Void visitFormalParameters(ShadowParser.FormalParametersContext ctx) {
		...
		for (ShadowParser.FormalParameterContext parameter : ctx.formalParameter()) {
			...
			if (names.contains(name))
				addError(parameter, Error.MULTIPLY_DEFINED_SYMBOL,
	...
	*/
			...
			if (list != null)
				for (ShadowParser.ClassOrInterfaceTypeContext child : list.classOrInterfaceType()) {
					...
					if (type instanceof InterfaceType)
						...
						addError(child, Error.INVALID_INTERFACE,
								kind + declarationType + " cannot implement non-interface type " + type, type);
				}
		}
			...
			if (list != null) {
				...
				for (ShadowParser.ClassOrInterfaceTypeContext child : list.classOrInterfaceType()) {
					...
					if (type instanceof ClassType) {
						...
						// exceptions.
						if (((declarationType.getClass() == ExceptionType.class) != (type
								.getClass() == ExceptionType.class)) || type.getClass() == SingletonType.class)
							// Nothing can extend a singleton.
							addError(child, Error.INVALID_PARENT,
							...
							addError(child, Error.INVALID_PARENT, kind + declarationType + " can only be child of "
									+ otherKind + type + " if " + type + " is listed first in the is list");
						else
						...
						addError(child, Error.INVALID_TYPE,
								kind + classType + " cannot extend or implement type " + type, classType, type);
	...
	public Void visitTypeParameters(ShadowParser.TypeParametersContext ctx) {
		...
		for (ShadowParser.TypeParameterContext typeParameter : ctx.typeParameter()) {
			...
			if (parameterNames.contains(name))
				addError(typeParameter, Error.MULTIPLY_DEFINED_SYMBOL,
	...
	public Void visitClassOrInterfaceType(ShadowParser.ClassOrInterfaceTypeContext ctx) {
		...
		if (ctx.getParent() instanceof ShadowParser.AttributeInvocationContext) {
			ShadowParser.AttributeInvocationContext attributeInvocation = (ShadowParser.AttributeInvocationContext) ctx.getParent();
			if (type instanceof AttributeType) {
				...
				addError(ctx, Error.INVALID_TYPE, "Cannot use non-attribute type " + typeName + " as an attribute");
			}
		} else if (type instanceof AttributeType) {
			// Attribute used in non-attribute-invocation
			addError(ctx, Error.INVALID_TYPE, "Attribute types can only be referenced before method declarations");
		}
