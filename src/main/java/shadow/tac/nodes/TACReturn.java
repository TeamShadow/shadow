...
/**
 * TAC representation of return statement Example: return 5
 *
 * @author Jacob Young
 */
public class TACReturn extends TACNode {
  ...
  public TACReturn(TACNode node, SequenceType expected, TACOperand op) {
    super(node);
    if (expected.isEmpty()) {
      if (op != null) throw new IllegalArgumentException("Operand is not equal to null: " + op);
    } else if (expected.size() == 1) returnValue = check(op, expected.get(0));
    else returnValue = check(op, new SimpleModifiedType(expected));
  }
  ...
  public TACOperand getOperand(int num) {
    if (num == 0 && hasReturnValue()) return getReturnValue();
