...
public class ShadowObject extends ShadowValue {
  ...
  public void setField(String name, ShadowValue value) throws InterpreterException {
    if (type.containsField(name)) {
      ...
      throw new InterpreterException(Error.INVALID_FIELD, "Type " + type + " does not contain field " + name);
  }
  ...
  public ShadowObject copy(Map<ShadowValue, ShadowValue> newValues) throws InterpreterException {
    if (newValues.containsKey(this))
      return (ShadowObject)newValues.get(this);
    ...
    ShadowObject replacement = new ShadowObject(getType(), parent == null ? null : parent.copy(newValues),  newFields);
  ...
  public ShadowValue[] callMethod(String method, ShadowValue... values)
    ...
    if (signature == null)
      throw new InterpreterException(Error.INVALID_METHOD, "Method " + method + "." + arguments + " not interpretable");
