...
public class Cleanup extends AbstractOutput {
  ...
  public static synchronized Cleanup getInstance() {
  ...
  protected void visit(TACNode node) throws ShadowException {
    if (node instanceof TACOperand) ((TACOperand) node).setData(null);
    for (TACOperand operand : node) operand.setData(null);
  }
}
